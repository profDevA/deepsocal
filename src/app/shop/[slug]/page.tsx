...
function ProductHero({ product }: { product: Product }) {
  ...
  return (
    <div className="relative grid grid-cols-1 md:grid-cols-[1fr_215px] gap-[20px] sm:gap-[28px] md:gap-[34px] lg:gap-[40px] p-[20px] sm:p-[40px] md:p-[60px] lg:p-[80px]">
      <div
        className="relative aspect-831/743 overflow-hidden"
        style={{
      ...
      >
        {cover ? (
          ...
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="font-acumin-condensed text-dark/30 text-[48px] sm:text-[64px] md:text-[80px] lg:text-[96px] tracking-[2px] uppercase select-none">
                {product.name}
              </span>
            </div>
          </>
        ...
        <div className="absolute right-[16px] sm:right-[20px] md:right-[26px] lg:right-[32px] bottom-[16px] sm:bottom-[20px] md:bottom-[26px] lg:bottom-[32px]">
          <span
            className="font-acumin-condensed text-white text-[16px] sm:text-[18px] md:text-[22px] lg:text-[24px] leading-none px-4 py-2 inline-block"
        ...
        {[0, 1, 2].map((i) => {
          ...
          return (
            <button
              ...
              className="relative shrink-0 size-[88px] sm:size-[120px] md:w-full md:h-auto md:aspect-square overflow-hidden cursor-pointer border border-[#c4c4c4] hover:border-dark transition-colors"
              style={{
                backgroundColor: product.themeColor,
                backgroundImage: src
                  ? undefined
  ...
  return (
    <div className="flex flex-col gap-4 self-start">
      ...
      <button
        ...
        className="bg-dark text-white font-acumin-condensed text-[18px] sm:text-[20px] md:text-[22px] lg:text-[24px] tracking-wide uppercase h-[44px] px-6 inline-flex items-center justify-center cursor-pointer transition-colors hover:bg-[#333] disabled:opacity-50 disabled:cursor-not-allowed border-none"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[20px] sm:gap-[28px] md:gap-[34px] lg:gap-[40px]">
        {products.map((p) => (
          <Link
            key={p.slug}
            href={`/shop/${p.slug}`}
            className="group block bg-[#dadada] overflow-hidden shadow-[0_4px_10px_rgba(0,0,0,0.1)] transition-transform hover:-translate-y-1 no-underline"
          >
            <article className="flex flex-col h-[360px] sm:h-[400px] md:h-[440px] lg:h-[470px]">
              ...
              >
                {p.images[0] ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="font-acumin-condensed text-dark/30 text-[28px] sm:text-[36px] md:text-[42px] lg:text-[48px] tracking-[1.5px] uppercase select-none">
                      {p.name}
                    </span>
                  </div>
                )}
                <span
                  className="absolute top-4 right-4 font-acumin-condensed text-white text-[14px] leading-[1.4] px-3 py-1 whitespace-pre"
                  style={{ backgroundColor: "rgba(0,0,0,0.6)" }}
                >
                  {`$ ${p.price.toFixed(2)}`}
                </span>
              ...
              <div className="bg-white h-[200px] px-[28px] py-[36px] flex items-end gap-4">
                <div className="flex flex-col gap-2 flex-1 min-w-0">
                  <h3 className="font-acumin-condensed text-[#303030] text-[24px] sm:text-[28px] md:text-[32px] leading-tight tracking-[1px] m-0 uppercase">
                    {p.name}
                  </h3>
